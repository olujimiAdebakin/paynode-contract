    ...
    ReentrancyGuardUpgradeable 
{
    ...
    event SettlementProposalRejected(
        bytes32 indexed proposalId,
        address indexed provider,
        string reason
    );

    event SettlementProposalTimeout(
        bytes32 indexed proposalId,
        address indexed provider
    );
     ...
     */
    function getProviderIntent(address _provider) 
        external view returns (ProviderIntent memory) 
    {
        ...
        returns (bytes32 orderId) 
    {
        ...
        orderId = keccak256(
            abi.encode(msg.sender, userNonce[msg.sender], block.chainid)
        );
        ...
        returns (bytes32 proposalId) 
    {
        ...
        // Validate fee
        require(
            _proposedFeeBps >= intent.minFeeBps && _proposedFeeBps <= intent.maxFeeBps,
            'InvalidFee'
        );

        // Generate proposal ID
        proposalId = keccak256(
            abi.encode(_orderId, _provider, block.timestamp, block.number)
        );
     ...
     */
    function timeoutProposal(bytes32 _proposalId) 
        external 
        onlyAggregator 
    {
        SettlementProposal storage proposal = proposals[_proposalId];
    ...
    function getProposal(bytes32 _proposalId) 
        external view returns (SettlementProposal memory) 
    {
     ...
     */
    function executeSettlement(bytes32 _proposalId) 
        external 
        onlyAggregator 
        nonReentrant 
    {
     ...
     */
    function refundOrder(bytes32 _orderId) 
        external 
        onlyAggregator 
        nonReentrant 
        validOrder(_orderId)
     ...
     */
    function _updateProviderSuccess(address _provider, uint256 _settlementTime) 
        internal 
    {
     ...
     */
    function flagFraudulent(address _provider) 
        external 
        onlyAggregator 
    {
     ...
     */
    function blacklistProvider(address _provider, string calldata _reason) 
        external 
        onlyOwner 
    {
    ...
    function getProviderReputation(address _provider) 
        external view returns (ProviderReputation memory) 
    {
